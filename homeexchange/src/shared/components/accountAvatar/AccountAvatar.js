...
class AccountAvatar extends PureComponent {
  ...
  render() {
    ...
    console.log('this.props.imgs', this.props.imgs[this.props.profileId]);
    const imgSrc = this.props.imgs[this.props.profileId] ?? bufSrc;
    return (
      <div className="account-avatar" onClick={this.handleClick.bind(this)}>
        <Avatar source={imgSrc} alternativeMessage="somthin" />
      </div>
    );
  }
}
